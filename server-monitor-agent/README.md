...
- Optional TCP network dependency checks
...
4. **Configure the agent:**
   ...
   SERVER_MONITOR_NETWORK_CHECKS_ENABLED=false
   SERVER_MONITOR_NETWORK_CHECK_INTERVAL_SECONDS=60
   SERVER_MONITOR_NETWORK_CHECK_TIMEOUT_MS=3000
   ```
...
| `iisLogs.allowlist.ipAddresses` | IPs excluded from suspicious IIS detections | `[]` |
| `iisLogs.allowlist.urlPathContains` | URL fragments excluded from suspicious IIS detections | `[]` |
| `iisLogs.allowlist.userAgents` | User-agent fragments excluded from suspicious IIS detections | `[]` |
| `networkChecks.enabled` | Enable lightweight agent-side TCP dependency checks | `false` |
| `networkChecks.scanIntervalSeconds` | How often to run configured TCP checks | `60` |
| `networkChecks.timeoutMs` | Per-check TCP timeout in milliseconds | `3000` |
| `networkChecks.checks` | Explicit TCP targets generated by WebMonitor | `[]` |
...
- **Network Checks**: Optional configured TCP port dependency checks
