@php
    ...
    $deploymentService = app(\App\Services\AgentDeploymentService::class);
    $previewConfig = $deploymentService->preview($server);
    $featureFlags = $previewConfig['featureFlags'] ?? config('agent.feature_flags', []);
    $defaultServices = $previewConfig['windowsServices'] ?? $deploymentService->profileWindowsServices($server);
    $profile = $previewConfig['deploymentProfile'] ?? null;
@endphp
...
@can('module.agent_deployment')
    <div class="flex flex-wrap gap-2">
        ...
        <a href="{{ route('servers.agent-package', $server) }}" onclick="return confirm('Generating a package rotates this server agent key. Deploy the new package before restarting the agent. Continue?');" class="inline-flex items-center justify-center rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100 dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
            Generate Package
        </a>
...
@else
    <div class="text-xs text-slate-500 dark:text-slate-400">Agent deployment actions require privileged access.</div>
@endcan
...
<dialog id="{{ $modalId }}" class="w-full max-w-3xl rounded-2xl border border-slate-200 bg-white p-0 text-slate-900 shadow-2xl backdrop:bg-slate-950/70 dark:border-white/10 dark:bg-slate-900 dark:text-slate-100">
    <form method="dialog" class="border-b border-slate-200 p-5 dark:border-white/10">
        <div class="flex items-start justify-between gap-4">
            <div>
                <h3 class="text-lg font-semibold">Generate Agent Config</h3>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $server->name }} / {{ $server->server_id }}</p>
                @if($profile)
                    <p class="mt-1 text-xs font-semibold uppercase tracking-wider text-orange-600 dark:text-orange-300">{{ $profile['name'] }}</p>
                @endif
            </div>
    ...
    <form method="GET" action="{{ route('servers.agent-config', $server) }}" class="space-y-5 p-5">
        <div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-100">
            Generating a config or package creates a new per-server key. Deploy the new file or package before restarting the agent; older per-server configs stop authenticating.
        </div>

        @if($profile)
            <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 dark:border-white/10 dark:bg-slate-950/60 dark:text-slate-200">
                <div class="font-semibold">Resolved profile: {{ $profile['name'] }}</div>
                <div class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    Roles: {{ filled($profile['roles'] ?? []) ? implode(', ', $profile['roles']) : 'none mapped yet' }}
                </div>
            </div>
        @endif
